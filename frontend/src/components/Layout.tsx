...
import { ArrowRight, Bot, LayoutDashboard, Sparkles, Ticket } from 'lucide-react'

import { ThemeToggle } from '@/components/ThemeToggle'
import { Button } from '@/components/ui/button'
import { cn } from '@/lib/utils'

export type AppView = 'input' | 'dashboard' | 'tickets' | 'actions'
...
export function Layout({ children, view, onNavigate, onNewAnalysis }: LayoutProps) {
  const showNav = view === 'dashboard' || view === 'tickets' || view === 'actions'
            ...
            {showNav && onNavigate && (
              <nav className="flex rounded-lg bg-muted p-1">
                <Button
                  variant="ghost"
                  size="sm"
                  className={cn('gap-2', view === 'dashboard' && 'bg-background shadow-sm')}
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className={cn('gap-2', view === 'tickets' && 'bg-background shadow-sm')}
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className={cn('gap-2', view === 'actions' && 'bg-background shadow-sm')}
